...
AR_agonist_venndiagram <- venn.diagram(list(ar_bla_agonist_p1=agonist1_top_listforvenndiagram, ar_mda_kb2_luc_agonist_p1=agonist2_top_listforvenndiagram, ar_mda_kb2_luc_agonist_p3=agonist3_top_listforvenndiagram),
                  ...
                  filename=NULL, 'cat.just' = list(c(1.35,-2),c(0.5,1),c(0.2,-3)) )
...
ggplot() + 
  ...
  labs(color = "Model") +
  scale_color_manual(values = (c("blue","red")), 
                     labels = ~ stringr::str_wrap(c("Full model (all chemotypes), accuracy: 0.9588106, auc: 0.8225909", "New model (only top chemotypes), accuracy: 0.9466815, auc: 0.5000000"), width = 20)) +
  geom_abline(lty=3) +
  coord_equal() +
  theme_bw() +
...
ggplot() + 
  ...
  labs(color = "Model") +
  scale_color_manual(values = (c("blue","red")), 
                     labels = ~ stringr::str_wrap(c("Full model (all chemotypes), accuracy: 0.9517049, auc: 0.8667600", "New model (only top chemotypes), accuracy: 0.9457971, auc: 0.5000000"), width = 20)) +
  geom_abline(lty=3) +
  coord_equal() +
  theme_bw() +
...
### Significant androgen receptor chemotypes 

Smarts.plus is a web based tool that can read SMARTS strings and convert them to visual representations. Since the chemotypes in the MACCS fingerprint are in SMARTS, this tool was used to visualize the results. Note that the ring structures (MACCS Fingerprint 96 and MACCS Fingerprint 145) were unable to be visualized, so the simplest carbon ring system was used to represent those chemotypes.

C..C..A..A (MACCS Fingerprint 26) ![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_26.png)

CC.C..C.A : (MACCS Fingerprint 66) ![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_66.png) A.A..A..A : (MACCS Fingerprint 105) ![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_105.png)

X5.M.ring : (MACCS Fingerprint 96)

![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_96.png)

C.C.C.C : (MACCS Fingerprint 50) ![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_50.png)

C.C : (MACCS Fingerprint 99) ![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_99.png)

C.C.A.A : (MACCS Fingerprint 76)
...
X6M.ring...1 : (MACCS Fingerprint 145)

![](https://raw.githubusercontent.com/AndreaHuacachino/BMIN503_Final_Project/master/chemotype_145.png)

### Limitations

The first potential limitation of the study was due to computational power. The models were created using 100 trees, and validated 5 cross fold.
...
Another limitation involves the final visualization of the chemotypes. The SMARTS system has dedicated programs to deciphering the chemical structure and properties that were not used in this project.

### Future Directions

These results are interesting and should be followed up on. A clear next step is to test the androgran receptor agonist models on unseen data, perhaps on all 15,000 known PFAS congers. The predictions from the model should also be validated experimentally, which is possible given the TOX21 public assay protocols. The interactions between the chemotypes and the androgen receptor should also be explored and visualized through molecular docking simulations.
